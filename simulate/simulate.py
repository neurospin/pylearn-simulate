...
class LinearRegressionData(SimulatedData):
    ...
    of Penalties function, i.e.:
        ...
        f(beta) = (1 / 2).|X.beta - y|²_2 + sum_{i=1}^N l_i.P_i(b),
    ...
    e : Numpy array (n-by-1). The error vector e = Xb - y. This vector carries
            the desired distribution of the residual. Note that if intercept is
            True, then e will be centred.

    snr : Positive float. Signal-to-noise ratio between model and residual.

    intercept : Boolean. Whether or not to include an intercept variable. If
            intercept is True, the first column of X0 must be a column of ones.
            This variable is not penalised. Note that if intercept is True,
            then e will be centred.
