...
ls = np.linspace(lambda_l1 - 0.25, lambda_l1 + 0.25, n_vals).tolist()
gs = np.linspace(lambda_gl - 0.25, lambda_gl + 0.25, n_vals).tolist()
...
plt.xlabel("$\lambda_{\ell_1}$", fontsize=14)
plt.ylabel("$\lambda_{\mathrm{GL}}$", fontsize=14)
plt.title(r"$f(\beta^{(k)}) - f(\beta^{*})$", fontsize=16)

x, y, _ = proj3d.proj_transform(lambda_l1, lambda_gl, np.min(Z), ax.get_proj())
...
def update_position(e):
    x, y, _ = proj3d.proj_transform(lambda_l1, lambda_gl, np.min(Z),
                                    ax.get_proj())
    label.xy = x, y
...
ax.set_xlim(lambda_l1 - 0.25, lambda_l1 + 0.25)
ax.set_ylim(lambda_gl - 0.25, lambda_gl + 0.25)
ax.set_zlim(0.0, 0.31)
